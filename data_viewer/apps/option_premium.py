...
class OptionPremium:
    def __init__(self, config: configparser.ConfigParser, app: dash.Dash):
        ...
        self.default_strike = {'NIFTY': 13500, 'BANKNIFTY': 30500}
                ...
                dbc.Row(
                    children=[
                        ...
                        dbc.Col(children=[html.A('center', href="#table-center-row"),
                                          html.Div(id='last-update-time1', children=[])],
                                width=3),
                        dcc.Interval(
                    ...
                    children=[
                        dbc.Col(
                            children=[
                                ...
                                dbc.Row(id='option-premium-table',
                                        children=[],
                                        style={
                                            'margin-top': 10,
                                            'padding': 10,
                        ...
                        dbc.Col(
                            children=[
                                dcc.Graph(id='option-premium-chart')
                            ], width=6)
                    ]),
    ...
    def register_callbacks(self, app: dash.Dash):
        ...
        @app.callback(
            Output('option-premium-chart', "figure"),
        ...
        def display_output(check_list_values, start_date, end_date, symbol, expiry, ):
            ...
            return self.get_premium(symbol, expiry, start_date, end_date, selected_strikes)

        @app.callback(
            [Output('option-premium-table', 'children'),
             Output('last-update-time1', 'children')],
    ...
    def get_premium(self, symbol, expiry, start_date, end_date, optionsList):
        def get_oi_data(symbol, expiry, start_date, end_date, strike, option_type):
            ...
            series.dropna(inplace=True, subset=['lastPrice'])
        ...
        for idx, option in enumerate(optionsList):
            trace_name += option if trace_name == '' else ('; '+ option)
            decoded_option = option.split('-')
            df = get_oi_data(symbol, expiry, start_date, end_date, decoded_option[0], decoded_option[1])
            if idx == 0:
                premium_df = df[['dateTime', 'underlyingValue', 'lastPrice']].copy()
            else:
                premium_df['lastPrice'] += df['lastPrice']

        layout = go.Layout(title='',
                           legend={"x": 0, "y": 1.2, 'orientation': "h"},
                           yaxis={'title': 'Premium', 'gridcolor': "#eeeeee", 'linecolor': "#636363", },
                           yaxis2={'title': 'Underlying', 'overlaying': 'y', 'side': 'right', 'gridcolor': "#eeeeee",
                                   'linecolor': "#636363",
        ...
        if premium_df is not None:
            fig.layout.xaxis.dtick = len(premium_df) / 15
            fig.add_trace(go.Bar(x=premium_df['dateTime'], y=premium_df['lastPrice'], name=trace_name, marker={'color': '#66bb6a'}))
            fig.add_trace(go.Scatter(x=premium_df['dateTime'], y=premium_df['underlyingValue'],
                                     hovertext='', yaxis='y2',
                                     name=symbol,
                                     mode='lines',
        ...
        # fig = px.bar(premium_df, x="dateTime", y="lastPrice",
        #               barmode="group") if premium_df else None
        return fig
